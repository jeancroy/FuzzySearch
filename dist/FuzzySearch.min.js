...
!function () {
    "use strict";
    function a(b) {
        return void 0 === b && (b = {}), this instanceof a ? void a.setOptions(this, b, a.defaultOptions, E, !0, this._optionsHook) : new a(b)
    }
    ...
    function j(a, b, c, d, e, f, g) {
        this.normalized = a, this.words = b, this.tokens_groups = c, this.fused_str = d, this.fused_map = e, this.fused_score = 0, this.has_children = f, this.children = g
    }
    ...
    function o(a) {
        for (var b = [], c = {}, d = a.fields, e = 0; e < d.length; e++)for (var f = d[e], g = 0; g < f.length; g++)for (var h = f[g], i = 0; i < h.length; i++)q(h[i], b, c);
        return b
    }

    function p(a) {
        var b, c, d = [], e = {}, f = a.words;
        for (b = 0; b < f.length; b++)q(f[b], d, e);
        var g = a.children;
        for (b = 0; b < g.length; b++)for (f = g[b].words, c = 0; f > c; c++)q(f[c], d, e);
        return d
    }

    function q(a, b, c) {
        var d = a.length;
        0 != d && (d >= 3 && s(a, 6, b, c), d >= 2 && r(a, 4, b, c), t(a[0], b, c))
    }

    function r(a, b, c, d) {
        for (var e = Math.min(a.length, b), f = 0; e - 1 > f; f++)for (var g = f + 1; e > g; g++)t(a[f] + a[g], c, d);
        return c
    }

    function s(a, b, c, d) {
        for (var e = Math.min(a.length, b), f = 0; e - 2 > f; f++)for (var g = f + 1; e - 1 > g; g++)for (var h = g + 1; e > h; h++)t(a[f] + a[g] + a[h], c, d);
        return c
    }

    function t(a, b, c) {
        a in c || (c[a] = !0, b.push(a))
    }
    ...
    function u(a, b) {
        ...
        for (var d = 0; d < a.length; d++) {
            var e = a[d];
            if (e in b)for (var f = b[e], g = 0; g < f.length; g++) {
                var h = f[g];
                h in c ? c[h]++ : c[h] = 1
            }
        }
        var i = [];
        for (var j in c)c.hasOwnProperty(j) && i.push(new v(j, c[j]));
        return i = i.sort(function (a, b) {
            return b.count - a.count
        })
    }

    function v(a, b) {
        this.id = a, this.count = b
    }
    ...
    function z(a, b, c, d) {
        ...
        for (e = 0; g > e; e++)h[e] = {};
        var i, j = new C(a, h, c, d), k = A(j, 0, 0).score, l = 0;
    ...
    function A(a, b, c) {
        ...
        for (j = 0; i > j; j++) {
            ...
            if (!(b & r || (k = d[c][j], m > k))) {
                if (p) {
                    l = b | r;
                    var s = l in q ? q[l] : A(a, l, c + 1);
                    k += s.score, j < s.index && (k += g)
                }
                k >= n && (n = k, o = j)
            }
        }
        p && (l = b, k = l in q ? q[l].score : A(a, l, c + 1).score, k > n && (n = k, o = -1));
        var t = new B(n, o);
        return e[c][b] = t, t
    }
    ...
    b.update = function (a, b, c) {
        ...
        setOptions: function (b, c) {
            void 0 === c && (c = b.reset || !1), a.setOptions(this, b, a.defaultOptions, E, c, this._optionsHook)
            ...
            (null === this.acro_re || "acronym_tok" in a) && (this.acro_re = f(b.token_sep)), (null === this.token_re || "token_sep" in a) && (this.token_re = b.token_re = new RegExp("[" + g(b.token_sep) + "]+", "g")), (a.dirty || "source" in a || "keys" in a || "use_index_store" in a) && (b.lazy ? this.dirty = !0 : (this._buildIndexFromSource(), this.dirty = !1))
        }
    }), c(a.prototype, {
        ...
        _prepQuery: function (b) {
            var c, d, e, f, g, h, i, k = this.options, l = k.score_per_token, m = k.score_test_fused, n = k.token_fused_max_length, o = k.token_field_min_length, p = k.token_field_max_length, q = this.tags, r = this.tags_re, s = q.length, t = this.token_re;
            if (l && s && r) {
                var u, v = 0, w = 0, x = new Array(s + 1), y = r.exec(b);
                for (g = null !== y; null !== y;)u = y.index, x[w] = b.substring(v, u), v = u + y[0].length, w = q.indexOf(y[1]) + 1, y = r.exec(b);
                x[w] = b.substring(v), f = [];
                for (var z = -1; ++z < s;) {
                    var A = x[z + 1];
                    A && A.length && (c = k.normalize(A), d = c.substring(0, n), e = m || !l ? a.alphabet(d) : {}, i = a.filterSize(c.split(t), o, p), h = a.pack_tokens(i), f[z] = new j(c, i, h, d, e, !1, []))
                }
                c = k.normalize(x[0]), i = a.filterSize(c.split(t), o, p), h = a.pack_tokens(i)
            } else c = k.normalize(b), i = a.filterSize(c.split(t), o, p), h = l ? a.pack_tokens(i) : [], g = !1, f = new Array(s);
            return d = c.substring(0, n), e = m || !l ? a.alphabet(d) : {}, new j(c, i, h, d, e, g, f)
        }
        ...
        var c = b.length;
        return c > F ? a.posVector(b) : a.bitVector(b, {}, 0)
    }, a.mapAlphabet = function (b) {
        for (var c = b.length, d = new Array(c), e = -1; ++e < c;) {
            var f = b[e];
            f.length > F ? d[e] = a.posVector(f) : d[e] = a.bitVector(f, {}, 0)
        }
        return d
        ...
        if (h > F)return g = a.llcs_large(b, c, d, m), l * g * g + j * m;
        var n, o, p = (1 << h) - 1, q = p;
        ...
        search: function (b) {
            ...
            this.start_time = c;
            var d = this.options;
            this.dirty && d.lazy && (this._buildIndexFromSource(), this.dirty = !1);
            var e = this.query = this._prepQuery(b), f = this.index, g = [];
            d.use_index_store && (f = this._storeSearch(e, f)), d.filter && (f = d.filter.call(this, f));
            var h = this._searchIndex(e, f, g);
            g = a.filterGTE(g, "score", h), "function" == typeof d.sorter && (g = g.sort(d.sorter)), (d.output_map || d.output_limit > 0) && (g = "function" == typeof d.output_map ? a.map(g, d.output_map, this, d.output_limit) : a.mapField(g, d.output_map, d.output_limit));
            var i = Date.now();
            return this.search_time = i - c, this.results = g, g
        ...
        _prepItem: function (b, c) {
            for (var d = a.generateFields(b, c), e = d.length, f = -1; ++f < e;)for (var g = d[f], h = -1, i = g.length; ++h < i;) {
                var j = this.options.normalize(g[h]), k = j.split(this.token_re);
                j.length > 2 * this.options.token_field_min_length && (k = a.filterSize(k, this.options.token_field_min_length, this.options.token_field_max_length)), this.options.score_acronym && k.push(j.replace(this.acro_re, "$1")), g[h] = k
            }
            return new m(b, d)
        }, add: function (a, b) {
            void 0 === b && (b = !0);
            var c, d = "function" == typeof this.options.identify_item ? this.options.identify_item(a) : null;
            null === d ? (c = this.nb_indexed, this.nb_indexed++) : d in this.index_map ? c = this.index_map[d] : (this.index_map[d] = this.nb_indexed, c = this.nb_indexed, this.nb_indexed++);
            var e = this._prepItem(a, this.keys);
            this.index[c] = e, b && (this.source[c] = a), this.options.use_index_store && this._storeAdd(e, c)
        }, _buildIndexFromSource: function () {
            var a = this.source.length;
            this.index = new Array(a), this.index_map = {}, this.nb_indexed = 0;
            for (var b = -1; ++b < a;) {
                var c = this.source[b];
                this.add(c, !1)
            }
        ...
        if (!b || !b.length)return [[a.toString()]];
        for (var c = b.length, d = new Array(c), e = -1; ++e < c;)d[e] = n(a, b[e].split("."), [], 0);
        return d
    }, c(a.prototype, {
        _storeAdd: function (a, b) {
            var c = o(a);
            if (0 != c.length)for (var d = 0; d < c.length; d++) {
                var e = c[d];
                e in this.store ? this.store[e].push(b) : this.store[e] = [b]
            }
        }, _storeSearch: function (b, c) {
            var d = p(b);
            if (0 == d.length)return [];
            var e = u(d, this.store);
            if (0 == e.length)return [];
            var f = .5 * e[0].count;
            return e = a.filterGTE(e, "count", f), a.map(e, function (a) {
                return c[a.id]
            })
        }
    });
    ...
    return a.filterSize = function (a, b, c) {
        ...
        y(c, j, o, p);
        var q = [], r = [], s = 0, t = 0;
        if (g && (t = a.matchTokens(n, m, r, d, !1)), (h || !g || i) && (s = a.score_map(k, l, a.alphabet(k), d) + d.bonus_token_order), 0 === t && 0 === s)return c;
        (!g || s > t) && (m = [k], n = [l], o = [c], r = [0]);
        for (var u = o.length, v = -1; ++v < u;) {
            var w = r[v];
            if (-1 !== w) {
                var x = m[w], z = n[v], A = o[v], B = 0, C = [], D = [];
                a.align(x, z, C, D);
                for (var E = C.length, F = -1; ++F < E;) {
                    var G = C[F], H = D[F];
                    G > B && q.push(A.substring(B, G)), q.push(e + A.substring(G, H) + f), B = H
                }
                q.push(A.substring(B) + p[v])
            } else q.push(o[v] + p[v])
        }
        return q.join("")
        ...
        var g, h, i, j, k, l, m, n = e.minimum_match, o = e.thresh_relative_to_best, p = [], q = b.length, r = c.length, s = a.mapAlphabet(b), t = n, u = -1, v = -1, w = 0, x = [];
        for (g = 0; q > g; g++)if (i = [], d[g] = -1, t = n, j = b[g], j.length) {
            for (l = s[g], h = 0; r > h; h++)k = c[h], k.length ? (m = a.score_map(j, k, l, e), i[h] = m, m > n && w++, m > t && (t = m, u = g, v = h)) : i[h] = 0;
            x[g] = t, p[g] = i
        } else {
            for (h = 0; r > h; h++)i[h] = 0;
            p[g] = i
        ...
        if (0 === w)return 0;
        if (1 === w)return d[u] = v, f && D(d, r), t;
        for (g = 0; g < b.length; g++)x[g] = Math.max(o * x[g], n);
        var y = z(p, d, x, e.bonus_token_order);
        return f && D(d, r), y
    }, c(a.prototype, {
